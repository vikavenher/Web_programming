...
function css() {
  return src(
    "app/css/*.css", // Беремо файли з розширенням css із папки app/css/
    { base: "app" }
...
function preprocessCss() {
  return src(
    `app/${use_preprocessor}` + // Беремо файли заданого препроцесора css із відповідної папки
      `/*.${use_preprocessor}`
  )
    .pipe(gulp_if(log, debug(opt))) // Відображаємо список оброблюваних файлів
...
function js() {
  return src(
    "app/js/*.js", // Беремо файли з розширенням js із папки app/js/
    { base: "app" }
  ) // Задаємо параметр base, щоб зберегти вложеність файлів
    .pipe(newer("build/")) // Відфільтровуємо лише змінені файли
...
function txt() {
  return src(
    "app/data/**/*.txt", // Беремо файли з розширенням txt із папки app/data/
    { base: "app" }
  ) // Задаємо параметр base, щоб зберегти вложеність файлів
    .pipe(newer("build/")) // Відфільтровуємо лише змінені файли
    .pipe(gulp_if(log, debug(opt))) // Відображаємо список оброблюваних файлів
...
function img() {
  return src(
    ["app/img/**/*.{png,jpg,jpeg,gif}", "app/data/**/*.{png,jpg,jpeg,gif}"], // Беремо файли з розширеннями png, jpg, jpeg, gif
    { base: "app" }
  ) // Задаємо параметр base, щоб зберегти вложеність файлів
    .pipe(newer("build/")) // Відфільтровуємо лише змінені файли
    .pipe(image_min({ verbose: log, silent: !log })) // Стискаємо зображення
    .pipe(dest("build/")); // Переміщуємо у папку build/
}
...
function watchForFiles() {
  ...
  watch("app/**/*.html").on("all", series(html, browser_sync.reload));

  // Слідкуємо за змінами css файлів
  watch("app/css/*.css").on("all", series(css));

  // Слідкуємо за змінами файлів препроцесора css - sass, less або scss
  watch(`app/${use_preprocessor}/*.${use_preprocessor}`).on(
    "all",
    series(preprocessCss)
  );

  // Слідкуємо за змінами js файлів
  watch("app/js/*.js").on("all", series(js, browser_sync.reload));

  // Слідкуємо за змінами txt файлів
  watch("app/data/**/*.txt").on("all", series(txt, browser_sync.reload));

  // Слідкуємо за змінами файлів зображень
  watch("app/img/**/*").on("all", series(img, browser_sync.reload));
}

// Публікуємо зібраний сайт на github
function deployOnGitHub() {
  return gh_pages.publish(
    "build", // Папка, вміст якої заливається на github
